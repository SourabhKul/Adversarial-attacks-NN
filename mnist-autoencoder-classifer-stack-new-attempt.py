...
for alpha in alpha_set:
    ...
    loss = 400*(tf.reduce_mean(tf.pow(y_true - y_pred, 2))) - tf.abs(tf.reduce_max(tf.nn.softmax(logits=target_pred_adv))-tf.reduce_max(tf.nn.softmax(logits=target_pred_true)))
    optimizer_a = tf.train.AdamOptimizer().minimize(loss)
    n = 10
...
def train_neural_network(x):
    ...
    optimizer_t = tf.train.AdamOptimizer().minimize(cost)
    #number for cycles
    hm_epochs = 15
    batch_size = 1000
    with tf.Session() as sess:
        ...
        for epoch in range(hm_epochs):
            ...
            for _ in range(int(mnist.train.num_examples/batch_size)):
                ...
                _, c = sess.run([optimizer_t,  cost], feed_dict={x:epoch_x, y:epoch_y})
                epoch_loss += c
        ...
        for i in range(1, num_steps+1):
            # Prepare Dataoptimizer_a
            # Get the next batch of MNIST data (only images are needed, not labels)
            batch_x, _ = mnist.train.next_batch(batch_size)
            
            # Run optimization op (backprop) and cost op (to get loss value)
            _, l = sess.run([optimizer_a, loss], feed_dict={X: batch_x})
        ...
        correct = tf.equal(tf.argmax(prediction,1), tf.argmax(y,1))
        accuracy = tf.reduce_mean(tf.cast(correct,'float'))
        print('Accuracy', accuracy.eval({x:g, y:mnist.test.labels}))
        # classification_orig = sess.run(tf.argmax(prediction, 1), feed_dict={x: batch_x})
        # classification_adv = sess.run(tf.argmax(prediction, 1), feed_dict={x: g})
        #print('orig:', classification_orig, ' adv:', classification_adv)
        #print('Accuracy for alpha', alpha, '=', (np.sum(classification_adv != y_label))/1000)
        for i in range(n):
            for j in range(n):
                # Draw the original digits
                canvas_orig[i * 28:(i + 1) * 28, j * 28:(j + 1) * 28] = \
                    batch_x[10*i+j].reshape([28, 28])
